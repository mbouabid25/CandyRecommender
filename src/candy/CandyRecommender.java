/**
*@author : Marwa Bouabid
*@version : 09/07/2020
*Disussed with Rhitom Mishra
*/
package candy;
import java.io.File;
import java.io.FileNotFoundException;
import java.util.ArrayList;
import java.util.HashSet;
import java.util.List;
import java.util.Scanner;
import java.util.Set;

/**
 * This program reads candies and their ingredients from a file.  It then asks 
 * the user which of those ingredients they like/dislike.  Finally, it rates 
 * the candies and tells the user which ones they would probably like.
 */
public class CandyRecommender {
    // All known candies
    private static List<Candy> candies = new ArrayList<>();
    
    // The ingredients the user likes
    private static List<String> likes = new ArrayList<>();
    
    // The ingredients the user dislikes
    private static List<String> dislikes = new ArrayList<>();

    // The ingredients the user loves
    private static List<String> loves = new ArrayList<>();
    
    // A map from ingredients to candies containing the ingredient.
    private static IngredientMap ingredientMap = new IngredientMap();



    /**
     * Read the candy information from the file called Candy.txt.  The file 
     * should be formatted like this:  candy name:ingredient 1, ingredient 2
     * Initializes the candies list and the ingredientMap.
     * @throws FileNotFoundException if the Candy.txt file cannot be found.
     */

    static void readCandyFile() throws FileNotFoundException {
        try (Scanner in = new Scanner (new File ("/Users/marwabouabid/Desktop/CS225/candy-recommender-mbouabid25/src/candy/Candy.txt"))) {
            // Read in each candy
            while (in.hasNextLine()) {
                String line = in.nextLine();
                String[] parsedLine = line.split(":");
                String name = parsedLine[0];
                String[] ingredients = parsedLine[1].split(",");
                Candy candy = new Candy(name, ingredients);
                candies.add(candy);
                
                // Add the information to the ingredient map.
                for (String ingredient : ingredients) {
                    ingredientMap.add(ingredient, candy);
                }
            }
        }
        System.out.println("Read " + candies.size() + 
                           " candies from the file.");
    }
    /**
     * For each ingredient found in any candy, ask the user if they like, 
     * dislike, or are neutral about that ingredient.
     */
     static void getUserPreferences() {
        try (Scanner in = new Scanner (System.in)) {
        
            // Ask the user about each ingredient
            for (String ingredient : ingredientMap.ingredients()) {
                /**This if statement makes sure that the computer only asks 
                *how much the user likes the ingredient once for each ingredient
                *which prevents the user to give two different outputs for the same ingredient
                */
            	if ((likes.contains(ingredient) == false) && (dislikes.contains(ingredient) == false) && (loves.contains(ingredient) == false))
            	{
                //Added a two in the possible inputs
                System.out.print ("How much do you like " + ingredient + 
                                  "? (1-like, 0-ok, -1-dislike, 2-love) ");
                String answer = in.nextLine();
                try {
                    int pref = Integer.parseInt(answer);
                    switch (pref) {
                    case 1:
                        likes.add(ingredient);
                        break;
                    case -1:
                        dislikes.add(ingredient);
                        break;
                    //Added a two in the possible inputs for "love"
                    case 2: 
                        loves.add(ingredient);
                    default:
                    }
                } catch (NumberFormatException e) {
                    System.out.println("Please enter 1, 0, -1 or 2");
                }
        }
    }
        }
     }

    /**
     * Recommend candies that you think the user will love based upon their
     * loves and dislikes and what the candies contain.  It will not suggest
     * any candidates that contain ingredients the user dislikes.  It will 
     * give a score to candies that contain only ingredients the user likes or loves.
     * A higher score is better.
     * @return the candies that this user would probably love
     */
        static Set<Candy> findLovedCandies() {
                //preferred, the candies that the user would probably love
                Set<Candy> preferred = new HashSet<>();
                System.out.println("PREFERRED CREATED");
                System.out.println("loves in candrec: " + loves);
                for (String ingredient : loves) {
                    System.out.println("ADDING CANDIES WITH " + ingredient);
                    for (Candy c : ingredientMap.getCandyWith(ingredient))
                        System.out.println("x " + c.getName());
                    preferred.addAll(ingredientMap.getCandyWith(ingredient));
                }
                for (String ingredient : dislikes) {
                	System.out.println("REMOVING CANDIES WITH " + ingredient);
                    preferred.removeAll (ingredientMap.getCandyWith(ingredient));
                }
                return preferred;
            }

    /**
     * Outputs the list of candies to the screen
     * @param possible the candies to output
     */
    private static void recommendLovedCandies(Set<Candy> preferred) {
    	System.out.println("The candy/ies you would love are :");
        for (Candy c: preferred) {
            System.out.println(c.getName() + ": " + c.score(likes,loves));
    }
        }

    /**
     * Reads the candies from a file, asks the user for their likes and 
     * dislikes, and makes a recommendation
     * @param args none
     */
    public static void main(String[] args) {
        try {
            readCandyFile();
            getUserPreferences();
            Set<Candy> lovedCandies = findLovedCandies();
            recommendLovedCandies(lovedCandies);
        } catch (FileNotFoundException e) {
            System.out.println ("Unable to read the Candy.txt file");
        }
    }
    
    
    /*helper methods I added to help controle the dislikes and the likes
    *cannot be private because has to be accessed by my test files
    */

    static void addDislike(String dislike) 
	{
	dislikes.add(dislike);
    
	}
    
    static void addLoves(String love) 
	{
	loves.add(love);
    
	}

}
